...
const SOURCES = [{
  name: "IEB — NSC past papers",
  href: "https://www.ieb.co.za/assessment/high-schools/national-senior-certificate/nsc-past-papers",
  ...
  name: "StudyPapers — IEB, all grades",
  href: "https://studypapers.co.za/ieb-past-papers-all-grades/",
  note: "Papers and memos sorted by grade and term"
}, {
...
/* first page of a pdf as a small jpeg, for the card previews */
...
function App() {
  ...
  const upload = async list => {
    ...
    for (const f of chosen) {
      ...
      let thumb = null;
      try {
        thumb = await makeThumb(await f.arrayBuffer());
      } catch {}
      const row = {
  ...
  const saveThumb = async (fileId, dataUrl) => {
    setFiles(prev => prev.map(x => x.id === fileId ? {
      ...x,
      thumb: dataUrl
    } : x));
    sb.from("files").update({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "mt-4 grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4"
    ...
    return /*#__PURE__*/React.createElement("button", {
      key: f.id,
      onClick: () => openFile(f),
      className: "shrink-0 max-w-[220px] text-left rounded-lg px-3 py-3 hover:-translate-y-1",
      style: {
        background: C.surface,
        border: `1px solid ${C.line}`,
    ...
    className: "relative text-left rounded-lg overflow-hidden hover:-translate-y-1",
    style: {
      background: C.surface,
      border: `1px solid ${C.line}`
    }
...
function FilesPanel({
  ...
  return /*#__PURE__*/React.createElement("div", null, folders.length > 0 && /*#__PURE__*/React.createElement("div", {
    ...
    className: "mt-5 grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4"
...
function FileRow({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "rounded-lg overflow-hidden hover:border-neutral-600",
    style: {
      background: C.surface,
      border: `1px solid ${C.line}`
    }
  ...
  }, file.thumb ? /*#__PURE__*/React.createElement("img", {
    src: file.thumb,
    alt: "",
    loading: "lazy",
    style: {
    ...
    style: {
      background: `linear-gradient(150deg, ${accent}22, #0F1216 70%)`
    }
  ...
  }, "opens to build preview")), file.paper && /*#__PURE__*/React.createElement("span", {
    className: "absolute top-2 left-2 rounded px-1.5 py-0.5 text-xs",
    style: {
      background: accent,
      color: "#000000",
      fontFamily: MONO
    }
  }, file.paper), bookmarks.length > 0 && /*#__PURE__*/React.createElement("span", {
    className: "absolute top-2 right-2 rounded px-1.5 py-0.5 text-xs",
    style: {
      background: "rgba(0,0,0,.6)",
      color: C.gold,
      fontFamily: MONO
    }
  }, bookmarks.length, " \u2605")), /*#__PURE__*/React.createElement("div", {
  ...
  }, showSubject && subject ? subject.name + " · " : "", fmtSize(file.size)), /*#__PURE__*/React.createElement("div", {
    className: "mt-2 flex items-center gap-0.5 no-print"
    ...
    style: {
      color: bookmarks.length ? accent : C.muted,
      border: `1px solid ${C.line}`,
      fontFamily: MONO
    }
  }, "\u2605"), /*#__PURE__*/React.createElement("span", {
    ...
    placeholder: "pg",
    inputMode: "numeric",
    className: "w-12 rounded-md px-2 py-1.5 text-sm outline-none",
    style: {
      background: C.raised,
      border: `1px solid ${C.line}`,
      color: C.text,
...
const KEEP_RENDERED = 5; // pages drawn either side of where you are
...
const hasMarks = d => !!(d && ((d.strokes || []).length || (d.notes || []).length || (d.images || []).length));
function scaleImage(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      const img = new Image();
      img.onload = () => {
        const r = Math.min(1, MAX_IMG_PX / Math.max(img.width, img.height));
        const c = document.createElement("canvas");
...
/* paint one page's marks into a 2d context sized w x h */
function paint(ctx, w, h, data, imgCache) {
  ...
  (data.notes || []).forEach(n => {
    ...
    const pad = fs * 0.5;
    const boxW = Math.min(maxW, Math.max.apply(null, lines.map(l => ctx.measureText(l).width))) + pad * 2;
    ...
    lines.forEach((l, i) => ctx.fillText(l, x + pad, y + pad + fs * (i + 0.85)));
    ctx.restore();
  });
}

/* ---------------- one page in the scroll ---------------- */
function PageView(props) {
  ...
  useEffect(() => {
    registerEl(index, boxRef.current);
  }, [index]);

  /* report which page you're looking at */
  useEffect(() => {
    if (!boxRef.current) return;
    const io = new IntersectionObserver(entries => {
      entries.forEach(e => {
        if (e.isIntersecting && e.intersectionRatio > 0.3) onEnter(index);
      });
    }, {
      threshold: [0.3]
    });
  ...
  const redraw = () => {
    ...
    if (over.width !== Math.floor(width * dpr)) {
      over.width = Math.floor(width * dpr);
      over.height = Math.floor(height * dpr);
    }
    ...
    if (live && live.pts.length > 1) paint(ctx, width, height, {
      strokes: [live]
    }, {});
  };

  /* render the pdf page only while it's near the viewport */
  useEffect(() => {
    let cancelled = false;
    if (!shouldRender) {
    ...
    (async () => {
      try {
        ...
        if (!base) return;
        base.width = Math.floor(vp.width * dpr);
        base.height = Math.floor(vp.height * dpr);
      ...
      } catch {/* cancelled render, ignore */}
    })();
    return () => {
      cancelled = true;
    };
  }, [doc, index, width, shouldRender]);
  useEffect(redraw, [data, width, height]);
  const norm = e => {
  ...
  const draft = noteDraft && noteDraft.page === index ? noteDraft : null;
  return /*#__PURE__*/React.createElement("div", {
    ref: boxRef,
  ...
  }, index), /*#__PURE__*/React.createElement("canvas", {
    ref: overRef,
  ...
  }, index), draft && /*#__PURE__*/React.createElement("div", {
    className: "absolute",
    style: {
      left: draft.x * 100 + "%",
      top: draft.y * 100 + "%",
...
function Viewer({
  ...
  const cacheImg = src => {
    ...
    const el = new Image();
    el.onload = () => setAnn(a => Object.assign({}, a)); // nudge a repaint once decoded
    el.src = src;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setSizes(dims);
        setStatus("");
        if (!file.thumb && onThumb) {
          try {
            const p1 = await pdf.getPage(1);
            const r1 = p1.getViewport({
              scale: 1
            });
            const v1 = p1.getViewport({
              scale: 320 / r1.width
            });
            const tc = document.createElement("canvas");
            tc.width = Math.floor(v1.width);
            tc.height = Math.floor(v1.height);
            await p1.render({
              canvasContext: tc.getContext("2d"),
              viewport: v1
            }).promise;
            onThumb(file.id, tc.toDataURL("image/jpeg", 0.72));
          } catch {}
        }
        const res = await sb.from("annotations").select("*").eq("file_id", file.id);
        if (dead) return;
        const map = {};
        (res.data || []).forEach(r => {
        ...
        setAnn(map);
        Object.keys(map).forEach(k => (map[k].images || []).forEach(im => cacheImg(im.src)));
      } catch {
  ...
  useEffect(() => {
    const measure = () => {
      const avail = (scrollRef.current ? scrollRef.current.clientWidth : 900) - 16;
      setColWidth(Math.max(240, Math.min(avail, 1000) * zoom));
    };
    measure();
    window.addEventListener("resize", measure);
    return () => window.removeEventListener("resize", measure);
  }, [zoom, doc]);
  const commit = (page, next) => {
    undoStack.current.push({
      page: page,
      snapshot: JSON.stringify(ann[page] || blankPage())
    });
    if (undoStack.current.length > 40) undoStack.current.shift();
    setAnn(a => Object.assign({}, a, {
      [page]: next
    }));
    clearTimeout(saveTimers.current[page]);
    saveTimers.current[page] = setTimeout(() => save(page, next), 700);
  };
  ...
  const undo = () => {
    ...
    clearTimeout(saveTimers.current[last.page]);
    saveTimers.current[last.page] = setTimeout(() => save(last.page, data), 400);
  };
  const addNote = draft => {
    ...
    const next = JSON.parse(JSON.stringify(ann[draft.page] || blankPage()));
    next.notes.push({
      x: draft.x,
      y: draft.y,
      text: draft.text.trim()
    });
    commit(draft.page, next);
    setNoteDraft(null);
  };
  const pickImage = (page, pt) => {
    imgTarget.current = {
      page: page,
      x: pt[0],
      y: pt[1]
  ...
  const addImage = async f => {
    if (!f) return;
    try {
      const out = await scaleImage(f);
      cacheImg(out.src);
      const t = imgTarget.current;
      const next = JSON.parse(JSON.stringify(ann[t.page] || blankPage()));
      next.images.push({
        x: t.x,
        y: t.y,
        w: 0.35,
  ...
  const exportFlat = async () => {
    ...
    try {
      ...
      const entries = Object.keys(ann);
      for (const key of entries) {
        const data = ann[key];
        const page = pages[Number(key) - 1];
        if (!page || !hasMarks(data)) continue;
        const dims = page.getSize();
        const c = document.createElement("canvas");
        c.width = Math.round(dims.width * 2);
        c.height = Math.round(dims.height * 2);
        ...
        paint(ctx, dims.width, dims.height, data, imgCache.current);
  ...
  const accent = subject && subject.color ? subject.color : C.gold;
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, doc ? current + " / " + doc.numPages : status || "…", saving ? " · saving" : "")), /*#__PURE__*/React.createElement("button", {
  ...
  }, status), doc && sizes.map((size, i) => /*#__PURE__*/React.createElement(PageView, {
    key: i + 1,
    doc: doc,
    index: i + 1,
    size: size,
    width: colWidth,
    data: ann[i + 1] || blankPage(),
    tool: tool,
    color: color,
    ...
    onEnter: setCurrent,
    shouldRender: Math.abs(i + 1 - current) <= KEEP_RENDERED
  }))), /*#__PURE__*/React.createElement("input", {
    ref: imgInput,
    type: "file",
    accept: "image/*",
  ...
  }, sizes.map((s, i) => /*#__PURE__*/React.createElement("option", {
    key: i + 1,
    value: i + 1
  }, "page " + (i + 1)))), bookmarks.filter(b => b.page).length > 0 && /*#__PURE__*/React.createElement("select", {
    value: "",
    onChange: e => {
      if (e.target.value) goTo(Number(e.target.value));
    },
    ...
    onClick: () => onAddBookmark(file, "Page " + current, current),
